  ...
  const handleSend = () => {
    ...
    setTimeout(() => {
      setMessages(prev => [...prev, { 
        role: 'assistant', 
        text: "Our artisans in Gulberg are currently trending for 'Bridal Glow' transformations. Would you like to see the top-rated sanctuaries?" 
      }]);
    }, 1000);
  };
            ...
            <div className="p-8 border-t border-primary/10 bg-white/40 dark:bg-black/40 backdrop-blur-md">
              <div className="relative flex items-center">
                <Input 
                  ...
                  className="rounded-full h-14 bg-white/80 dark:bg-white/5 border-primary/10 px-8 italic font-body focus-visible:ring-primary/20"
                />
                <Button 
                  onClick={handleSend}
                  size="icon" 
                  className="absolute right-2 h-10 w-10 rounded-full bg-primary text-primary-foreground"
                >
