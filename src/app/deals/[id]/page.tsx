...
import { MapPin, Star, ArrowRight, ArrowLeft, ShieldCheck, Users, Plus, Minus, Info, MessageCircle } from 'lucide-react';
...
export default function DealPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!isMounted || isLoadingDeal) return null;
      ...
      <main className="container mx-auto px-6 py-12">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-20 items-start">
          <div className="space-y-6">
            <div className="relative aspect-square bg-muted rounded-3xl overflow-hidden shadow-2xl">
          ...
          <div className="space-y-12">
            ...
            <div className="space-y-8">
              <div className="flex items-baseline gap-4">
                ...
                <span className="text-xl text-muted-foreground line-through opacity-30">{getCurrency()} {deal.basePrice.toLocaleString()}</span>
              </div>
              
              <div className="p-8 border rounded-3xl flex items-center justify-between bg-white/40 backdrop-blur-xl shadow-xl">
                <div className="space-y-1">
                  <div className="flex items-center gap-2 text-primary font-black uppercase text-[10px] tracking-widest">
                    <Users className="h-4 w-4" /> Guest Count
                  </div>
                  <p className="text-[10px] text-muted-foreground italic">Book for you and friends</p>
                </div>
                <div className="flex items-center gap-6 bg-white/60 px-6 py-2 rounded-full border">
                  <Button variant="ghost" size="icon" onClick={() => setPersonCount(Math.max(1, personCount - 1))}><Minus className="h-3 w-3" /></Button>
                  <span className="font-headline text-4xl text-primary">{personCount}</span>
                  <Button variant="ghost" size="icon" onClick={() => setPersonCount(personCount + 1)}><Plus className="h-3 w-3" /></Button>
                </div>
              </div>

              <div className="p-10 rounded-3xl bg-secondary/30 backdrop-blur-md space-y-4">
                <div className="flex justify-between items-baseline italic text-primary">
                  <span className="text-sm font-bold">Deposit Due Now</span>
            ...
            <Button size="lg" className="w-full h-20 bg-primary text-primary-foreground rounded-3xl text-xl font-bold uppercase tracking-widest text-[10px] shadow-2xl group" onClick={handleAddToCart}>
              Book Now
